...
extension UIDevice {
    ...
    var type: Model {
        ...
        guard let mcode = modelCode, let map = .some(mcode), let model = modelMap[map] else { return Model.unrecognized }
        if model == .simulator {
            if let simModelCode = ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] {
                if let simMap = .some(simModelCode), let simModel = modelMap[simMap] {
                    return simModel
                }
            }
        }
